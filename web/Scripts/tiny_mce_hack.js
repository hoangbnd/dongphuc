...
(function ($, tinymce) {
    ...
    var overrideOpen = function (proto) {
        ...
        proto.open = function () {
            ...
            if (param) {
                var url = param.url || param.file;
                if ($.type(url) === 'string') {
                    var lowerUrl = url.toLowerCase();
                    var imageFlag = '/image.htm'; // end with
                    if (lowerUrl.indexOf(imageFlag) === lowerUrl.length - imageFlag.length) {
                        arguments[0].width += 50;
                        this.onOpen.add(onDialogOpen);
                    }
                    var mediaFlag = '/media.htm'; // end with
                    if (lowerUrl.indexOf(mediaFlag) === lowerUrl.length - mediaFlag.length) {
    ...
    var injectMediaLibrary = function (win, editor) {
        ...
        temp.innerHTML = '<img title="Media Library" style="cursor:pointer;" src="/Styles/images/icons/add.png" />';
